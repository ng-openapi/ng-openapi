...
- **Barrels are re-exported for you.** If your plugin writes an `index.ts` into its own directory directly under the output root (as in the example above), the generated root `index.ts` automatically re-exports that directory — users can import your symbols from the client entrypoint.
