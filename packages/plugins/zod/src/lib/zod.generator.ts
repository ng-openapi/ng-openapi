// packages/plugins/zod/src/lib/zod.generator.ts
...
const DEFAULT_OPTIONS: ZodPluginOptions = {
    strict: false,
    coerce: false,
    generate: {
        param: true,
...
export class ZodGenerator implements IPluginGenerator {
    ...
    private async generateValidatorFile(
    ...
    private generateHeaderComment(validatorName: string): string {
        return `/**
 ...
 */\n\n`;
    ...
    private async generateOperationValidators(sourceFile: SourceFile, operation: PathInfo) {
        ...
        if (this.shouldGenerate('body') && operation.requestBody) {
            const bodyValidators = await this.schemaGenerator.generateBodyValidator(
                operation,
                operationName
            );
        ...
        if (this.shouldGenerate('response')) {
            const responseValidators = await this.schemaGenerator.generateResponseValidators(
                operation,
                operationName
            );
    ...
    private async generateParameterValidators(
        operation: PathInfo,
