...
import { PropertyAssignmentStructure, StructureKind } from "ts-morph";

/**
 * Returns the initial value for a form control as a string.
 * @param p The form property.
 * @returns A string representation of the initial value (e.g., "null", "''", "[]").
 */
...
/**
 * Generates an array of `PropertyAssignmentStructure` for building a FormGroup programmatically.
 * This is the core engine for creating the structure of the `form` property in components.
 * @param properties - An array of `FormProperty` definitions for the form.
 * @param parentModelName - The name of the parent model/interface for strong typing.
 * @returns An array of structures that `ts-morph` can use to build object literals.
 */
export function generateFormControlsTS(properties: FormProperty[], parentModelName: string): PropertyAssignmentStructure[] {
    return properties.map((p): PropertyAssignmentStructure => {
        let initializer: string;

        if (p.type === 'object' && p.nestedProperties) {
            const nestedControls = generateFormControlsTS(p.nestedProperties, p.nestedObjectTypeName!);
            const propertiesText = nestedControls.map(prop => `${prop.name}: ${prop.initializer}`).join(',\n    ');
            initializer = `new FormGroup({
    ${propertiesText}
})`;
        } else if (p.type === 'polymorphic' && p.polymorphicOptions) {
            const subForms = p.polymorphicOptions.map(opt => {
                const nestedControls = generateFormControlsTS(opt.properties, opt.name);
                const propertiesText = nestedControls.map(prop => `${prop.name}: ${prop.initializer}`).join(',\n            ');
                return `'${opt.name}': new FormGroup({
            ${propertiesText}
        }, { disabled: true })`;
            }).join(',\n        ');

            initializer = `new FormGroup({
        'typeSelector': new FormControl(null${p.required ? ', Validators.required' : ''}),
        ${subForms}
    })`;
        } else if (p.type === 'array_object' || p.type === 'array') {
            const validators = p.validators.length > 0 ? `, { validators: [${p.validators.join(', ')}] }` : '';
            const initialControls = (p.defaultValue && Array.isArray(p.defaultValue))
                ? `[${p.defaultValue.map(val => `new FormControl(${JSON.stringify(val)})`).join(', ')}]`
                : '[]';
            initializer = `new FormArray(${initialControls}${validators})`;
        } else if (p.type === 'file') {
            const validators = p.validators.length > 0 ? `, { validators: [${p.validators.join(", ")}] }` : '';
            initializer = `new FormControl<File | null>(null${validators})`;
        } else { // Handles string, number, boolean, enum
            ...
            initializer = `new FormControl<${typeArgument}>(${initialValue}${optionsString})`;
        }
...
/**
 * Generates the HTML string for form fields based on `FormProperty` definitions.
 * This function is used to populate the component's HTML template.
 * @param properties - An array of `FormProperty` definitions.
 * @param isViewMode - If true, generates a read-only details list (`<dl>`) instead of form inputs.
    ...
    return properties.map(p => {
        ...
        switch (p.inputType) {
            ...
            case 'file':
                ...
                <input type="file" class="hidden-file-input" #fileInput${pascalCase(p.name)} (change)="onFileSelected($event, '${p.name}')">
                <button mat-stroked-button type="button" (click)="fileInput${pascalCase(p.name)}.click()"><mat-icon>attach_file</mat-icon>Choose File</button>
                <span class="file-name">{{ form.get('${p.name}')?.value?.name || "No file chosen" }}</span></div>`;
        }
        switch (p.type) {
            case 'object':
            ...
            case 'array_object':
                ...
                return `<div class="form-array-container"><div class="form-array-header"><h4>${label}</h4><button type="button" mat-stroked-button (click)="add${singularTitle}()">
                 <mat-icon>add</mat-icon> Add ${singularTitle}</button></div><div formArrayName="${p.name}">
                 @for (item of ${p.name}.controls; track $index; let i = $index) {
